...
def make_td_diff(n, s, nr,data=2):
    ...
    state0 = np.random.randint(0, 2**64, size=(n, 5*s), dtype=np.uint64)
    diff = np.zeros(5*s, dtype=np.uint64)
    diff[np.arange(0, 5*s, 5)] = diff_arr[0]
    diff[np.arange(1, 5*s, 5)] = diff_arr[1]
    diff[np.arange(2, 5*s, 5)] = diff_arr[2]
    diff[np.arange(3, 5*s, 5)] = diff_arr[3]
    diff[np.arange(4, 5*s, 5)] = diff_arr[4]
    state1 = state0 ^ diff
    num_rand_samples = np.sum(Y == 0)
    state1[Y == 0, :] = np.random.randint(0, 2**64, size=(num_rand_samples, 5*s), dtype=np.uint64)
    state0 = state0.reshape(n*s, 5)
